...
def draw_color_wheel_np(width, height):
    color_wheel_x = np.linspace(-width / 2.0, width / 2.0, width)
    color_wheel_y = np.linspace(-height / 2.0, height / 2.0, height)
...
def flow_viz_np2(flow_x, flow_y):
    ...
    ang *= 180.0 / np.pi / 2.0
    ang = ang.astype(np.uint8)
    hsv = np.zeros([flow_x.shape[0], flow_y.shape[1], 3], dtype=np.uint8)
    hsv[:, :, 0] = ang
    hsv[:, :, 1] = 255
    hsv[:, :, 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
    flow_rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
...
def flow_polar(flow_x, flow_y):
    ...
    ang *= 180.0 / np.pi / 2.0
    ang = ang.astype(np.uint8)
    hsv = np.zeros([flow_x.shape[0], flow_y.shape[1], 2], dtype=np.uint8)
    hsv[:, :, 0] = ang
